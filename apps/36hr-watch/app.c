 ...
 */
void app_init(void) { memset(&board, 0, sizeof(board)); }
...
void app_setup(void) {
  ...
  watch_register_interrupt_callback(BTN_MODE, cb_mode_pressed,
                                    INTERRUPT_TRIGGER_RISING);
  watch_register_interrupt_callback(BTN_LIGHT, cb_light_pressed,
                                    INTERRUPT_TRIGGER_RISING);
...
bool app_loop(void) {
  // play a beep if the mode has changed in response to a user's press of the
  // MODE button
  ...
  if (board.led_on && board.light_ticks == 0) {
    // unless the user is holding down the LIGHT button, in which case, give
    // them more time.
...
void do_36_hr_clock_mode(void) {
  watch_36hr_date_time date_time =
      watch_date_time_to_36hr(watch_rtc_get_date_time());
  const char months[12][3] = {"JA", "FE", "MR", "AR", "MA", "JN",
                              "JL", "AU", "SE", "OC", "NO", "dE"};
...
void set_time_mode_handle_secondary_button(void) {
  watch_date_time date_time = watch_rtc_get_date_time();
  const uint8_t days_in_month[12] = {31, 28, 31, 30, 31, 30,
                                     30, 31, 30, 31, 30, 31};
  ...
  case 5: // day
    ...
    // can't set to the 29th on a leap year. if it's february 29, set to 11:59
    // on the 28th. and it should roll over.
