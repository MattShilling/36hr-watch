...
watch_36hr_date_time to_36_hr_date(uint32_t year, uint32_t month, uint32_t day,
  ...
  // The last day of each 36hr month might not actually have 36hrs in it (a
  // short day). Check if the day is a short day and handle accordingly.
  ...
  // Total number of hours from the beginning of the current month until the
  // current _hour_ of the current day.
  uint32_t total_hrs_elapsed = (day * 24) + hour;
  uint32_t hour_converted =
      total_hrs_elapsed % get_num_hours_in_day(year, month, day_converted);
